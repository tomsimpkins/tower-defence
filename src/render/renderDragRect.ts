...
import type { InteractionsSlice, IStore, TowerSlice } from "../store";

import { getTowerUiSpec } from "../ui/towers";
...
export const renderDragOverlay =
	(store: IStore<TowerSlice & InteractionsSlice>) =>
	(world: World, ctx: CanvasRenderingContext2D) => {
		const state = store.getState();
		if (state.interactions.type !== "dragging") {
			return;
		}

		const sx = state.interactions.startWorld.x;
		const sy = state.interactions.startWorld.y;
		const cx = state.interactions.currentWorld.x;
		const cy = state.interactions.currentWorld.y;
		...
		const drawTower = getTowerUiSpec(state.tower.active).draw;
		ctx.globalAlpha = 0.6;
		for (let x = gx0; x < gx1; x += CELL_SIZE) {
			for (let y = gy0; y < gy1; y += CELL_SIZE) {
				const isAvailable = world.resource.grid.isAvailable(
					(x / CELL_SIZE) | 0,
					(y / CELL_SIZE) | 0,
				);
				...
				// Draw tower faintly
				ctx.save();
				ctx.globalAlpha = !isAvailable ? 0.4 : 0.6;
				drawTower(ctx, x + padding, y + padding);
				ctx.restore();
